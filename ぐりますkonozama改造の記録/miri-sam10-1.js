...
//		num = window.prompt('アイドル番号を入力してください\n(1春香-50ジュリアまで)', '10');
    if(typeof num !== 'number'){
			$('<div/>').css({
				...
				textAlign: 'center',
				paddingTop: '15em'
			}).attr('id', '___overlay').text('アイドルちゃんランキング10位集計').appendTo('body');
      num = 1;
      //alert('開始します');
    }
...
//      init(num+1);
			...
			for(i=0;i<50;i++){
			  ...
			  finishdata += idolname[i]+ '10位\t' + fundata[i]  + ownfundata[i] +'\n';
			}
			//alert(finishdata);
