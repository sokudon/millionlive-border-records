(function(){
	...
	var idolname=[];
	for(var i=0;i<37;i++){idolname[i]=i+2203;}
...
if (MM < 10) { MM = '0' + MM;}
	...
	function load(idol) {
		...
		page.done(function(data){
		  ...
		  var dataown = null;
			...
			$(dom).find(' div.f-card-head').each(function(){
				temp = $(this).text();
          		data100 += idol + "\t\t"
          				+temp
          				+ "\r\n"	;
	...
	function get(idol,idol) {
		var df = $.Deferred();
		$.ajax({
			url: 'http://imas.gree-apps.net/app/index.php/album/detail/id/'+ idol+'/type/2/attribute/0/rare/0',
