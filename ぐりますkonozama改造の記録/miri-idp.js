(function() {
    ...
    var google ="https://script.google.com/macros/s/AKfycbxLvTzJjHy-Kvk8KHefY0fLpeig5kG8DazT-ZG0C-eU6OL_QwA/exec"
	...
	'http://imas.gree-apps.net/app/index.php/ranking/idol/past/1'
	]
    ...
    function init(num) {
        ...
        progress.done(function(data100, dataown) {
            ...
            if (num > idolname.length) {
                ...
                var dd = new Date();
                finishdata += "※" + dtstring(dd) + ' 集計時点 の先週ランキング'+ credit +'\r\n';
                //alert(finishdata);
                ...
                TextDL(finishdata, dtstring(dd).replace(/[/ :\-]/g, "")+"-idrank");
...
//request = $.ajax({url: google,type: "post",	data: BD});
    ...
    var r=st.match(/位\t(.*?)\t/);
    var m=st.replace(/,/gm,"").match(/[0-9]+人/);
    JS[r[1]]=m[0].replace("人","");
     }
    ...
    function load(idol) {
        ...
        page.done(function(data) {
            ...
            $(dom).find('table').each(function() {            
                temp = $(this).html();
                if (temp.match(/txt-sub2/) != null) {            
                    data100 +=  $(this).text().replace(/\t/gm,"")
                    .replace(/\n+/gm,"\t")
                    .replace(/^\t/gm,"")
                    .replace(/\t$/gm,"")
                    + "\r\n";
