...
//http://imas.gree-apps.net/app/index.php/ranking/lounge/segment/2/rank_page/1
   ...
   //d.writeln("<pre>"+finishdata +"</pre>");
                ...
                TextDL(finishdata, "fan-full3i-"+dtstring(dd).replace(/[/ :\-]/g, ""));


				finishdata=finishdata.replace(/\t.*?\t.*?\t/gm,"\t");
				finishdata=finishdata.replace(/\-.+/gm,"--");
				finishdata=addc(finishdata);
                TextDL(finishdata, "fan-full3i-diff-"+dtstring(dd).replace(/[/ :\-]/g, ""));
    ...
    function load(id,idol) {
        ...
        page.done(function(data) {
            ...
            $(dom).find('td.user-list-st').each(function() {            
                temp = $(this).html();
                if (temp.match(/txt-sub2/) != null  && temp.match(/位/)!= null && temp.match(/現在の順位/)== null) {
    ...
    function get(id, idol) {
        ...
        $.ajax({
            ...
            url: uribase + id +'/rank_page/'+idol,//+"/past/1",
